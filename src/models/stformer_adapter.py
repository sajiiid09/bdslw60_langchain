...
class STFormerFromCodes(nn.Module):
    def __init__(
        self,
        stformer_kwargs: dict,
        ...
        stformer_kwargs = dict(stformer_kwargs)
        stformer_kwargs["input_channels"] = self._adapter_channels
        self._stformer_kwargs = stformer_kwargs
        self.backbone = STFormer(**stformer_kwargs)
    ...
    def init_adapter(self, input_feature_dim: int) -> None:
        self.adapter = SequenceToVideoAdapter(
            ...
            target_num_frames=self._stformer_kwargs.get("num_frames", 8),
            target_image_size=self._stformer_kwargs.get("image_size", 224),
        )
